...
public class Pedestrian {
    ...
    private static String MEDIA_URL; //= Comuna.class.getResource("oh_no_short.wav").toExternalForm();
    ...
    public Pedestrian (Comuna comuna, double speed, double deltaAngle, int state, double I_time, boolean mask){
        ...
        if (comuna.sonidoarg) {
            MEDIA_URL = Comuna.class.getResource(comuna.audiofile).toExternalForm();
    ...
    {
        newState = state;
        if(state == 0) this.verifyVac();
        //if(state == 3) return newState;
            ...
            if (auxD <= powD) {
                if (this.getMask() && person2.getMask()){
                    if (Math.random() < P2) {
                        ...
                        if (comuna.sonidoflag && comuna.sonidoarg) {
    ...
    public void verifyVac()
    {
        ArrayList<Vacunatorio> vacunatorios = comuna.getVac();
        double VacSize = SimulatorConfig.VAC_SIZE;

        for(int i = 0; i < vacunatorios.size(); i++)
        {
            if ((this.x < (vacunatorios.get(i).getVacX()) + VacSize + 0.5*comuna.PedSize) &&
                    (this.x > vacunatorios.get(i).getVacX()-0.5*comuna.PedSize) &&
                    (this.y > vacunatorios.get(i).getVacY()-0.5*comuna.PedSize) &&
                    (this.y < (vacunatorios.get(i).getVacY()) + VacSize + 0.5*comuna.PedSize) )
            {
                state = 3;
