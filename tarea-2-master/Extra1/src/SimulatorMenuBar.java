...
public class SimulatorMenuBar extends MenuBar {
    ...
    private void Settings(Boolean option, Simulator simulator, SimulatorConfig simulatorC, Menu settings) throws IOException {
        if (option) {
            ...
            controller.sound.setOnMouseClicked(new ToggleHandler(controller.sound, controller, this.simulator));
    ...
    public class ToggleHandler  extends Exception implements EventHandler<MouseEvent> {


        private ToggleButton toggle;
        private settingsControl controller;
        private Simulator simulator;

        public ToggleHandler(ToggleButton toggleButton1, settingsControl controller, Simulator simulator) {
            this.controller = controller;
            this.toggle =toggleButton1;
            this.simulator = simulator;
        }

        @Override
        public void handle(MouseEvent click) {
            this.simulator.getComuna().sonidoflag = controller.sound.isSelected();
