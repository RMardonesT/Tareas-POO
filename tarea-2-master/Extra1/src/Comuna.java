...
public class Comuna {
    ...
    private ArrayList<Vacunatorio> vacunatorio;
    private VacView vacView;
    private StackPane graph;
    public NumberAxis ejeX;
    public NumberAxis ejeY;
    public StackedAreaChart<Number, Number> grafico;
    //Crear "Series" para V, I, R, S (abajo hacia arriba)
    private XYChart.Series<Number, Number> dataV;
    ...
    public final double PedSize = 20;
    public boolean vacnotCreated;
    static public boolean sonidoflag;
    final public boolean sonidoarg;
    final public String audiofile;
    private  double N;


    //Constructor
    public Comuna(boolean sonidoarg, String audiofile) {
        this.audiofile = audiofile;
        this.sonidoarg = sonidoarg;
        this.sonidoflag = true;
        ...
        dataV = new XYChart.Series<Number, Number>();
        ...
        N = SimulatorConfig.N;
        vacunatorio = new ArrayList<Vacunatorio>();
        //for(int i=0; i < SimulatorConfig.NUM_VAC; i++) {
        //    vacunatorio.add(new Vacunatorio(SimulatorConfig.VAC_SIZE, this));
        //}
    }
    ...
    public void createGraph(){
        ...
        dataV.getData().add(new XYChart.Data(0,0));
    ...
    public void Interaction(){
        for (int i= 0; i < people.size()  ; i++){
            ...
            if (person1.getState() !=2 && person1.getState() !=3) {
    ...
    public void ResetPedestrian(){
        this.getView().getChildren().remove(1, 2*(int)this.N+1); //Borro los anteriores al actualizar los parametros
        N = SimulatorConfig.N;  //N actualizado
        people = new ArrayList<Pedestrian>();
        populate_comuna();
    }

    public void ResetVac(){
        if (!vacnotCreated) this.getView().getChildren().remove(1, (int)SimulatorConfig.NUM_VAC+1); //Borro los anteriores al actualizar los parametros
        vacunatorio = new ArrayList<Vacunatorio>();
        //for(int i=0; i < SimulatorConfig.NUM_VAC; i++) {
        //    vacunatorio.add(new Vacunatorio(SimulatorConfig.VAC_SIZE, this));
        //}
    ...
    public void getState(double time) {
        ...
        for (int i=0; i< N; i++)
        {
            ...
            if (people.get(i).state == 3) V++;
        }
        dataV.getData().add(new XYChart.Data(time/10, V));
    ...
    public ArrayList<Vacunatorio> getVac(){
        return vacunatorio;
    }
