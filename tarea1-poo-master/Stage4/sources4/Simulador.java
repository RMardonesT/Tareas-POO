...
public class Simulador {
    ...
    private double N, I, I_time, speed, delta_t, deltaAngle, samplingTime, d, p0, M, p1, p2, Num_Vac, VacSize, Vac_Time;
    ...
    public Simulador(PrintStream output, Comuna comuna, double[] parameters) {
        ...
        this.M = parameters[9];
        this.p1 = parameters[10];
        this.p2 = parameters[11];
        this.Num_Vac = parameters[12];
        this.VacSize = parameters[13];
        this.Vac_Time = parameters[14];
        vacunatorios = new ArrayList<Vacunatorio>();
    }

    // IMPRIME LOS  TITULOS LAS COLUMNAS ASOCIADA A LA SIMULACION  T V I R S
    ...
    //IMPRIME LOS LOS VALORES DE LAS COLUMANS T V I R S
    ...
    private ArrayList populate_comuna() {
        ...
        boolean Mask;
        int num_maskInf = (int) Math.round(I * M);
        int num_maskSus = (int) Math.round((N - I) * M);
        for (int i = 0; i < N; i++) {
            ...
            if (i < num_maskInf || (i >= I && i < I + num_maskSus)) {  //incluye M*(N-I) personas usando mascarillas
                Mask = true;
            } else {
                Mask = false;
            }

            Individuo person = new Individuo(comuna, speed, deltaAngle, state, I_time, Mask); //inicializa un individuo
            arr_people.add(person); //Agrega el individuo al arreglo de personas
        }
        return arr_people;
    }
    ...
    private void update_comuna(ArrayList <Individuo> arr_people)
    {
        ...
        {
            if (!arr_people.get(i).getStateVac())
            {
                if (arr_people.get(i).getStateSus())
                {
                    arr_people.get(i).verifyVac(vacunatorios, VacSize);
                    if (arr_people.get(i).getStateVac())
                    {
                        continue;
                    }
                    ...
                    else
                    {
                        for (int j = 0; j < arr_people.size(); j++)
                        {
                            if(arr_people.get(j).getStateInf())
                            {
                                ...
                                if (arr_people.get(i).getMaskState() && arr_people.get(j).getMaskState())
                                {
                                    P = p2;
                                }
                                else if (arr_people.get(i).getMaskState() || arr_people.get(j).getMaskState())
                                {
                                    P = p1;
                                }
                                if (arr_people.get(i).getSusNewState(arr_people.get(j), d, P)) {
                                    break;
                                }
                            }
                        }
    ...
    private void setVac ()
    {
        ...
        {
            vacunatorios.add(new Vacunatorio(VacSize, comuna));
        }
    }
