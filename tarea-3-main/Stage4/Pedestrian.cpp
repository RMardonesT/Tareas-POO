...
void Pedestrian::verifyvac(){
    Vacunatorio *cursor_vac = &comuna.own_vacs->front();
    while(cursor_vac != &comuna.own_vacs->back()){
        if((x < (cursor_vac->getVacX() + comuna.parameters[14])) &&
           (x > cursor_vac->getVacX()) &&
           (y < (cursor_vac->getVacY() + comuna.parameters[14])) &&
           (y > cursor_vac->getVacY()) &&
